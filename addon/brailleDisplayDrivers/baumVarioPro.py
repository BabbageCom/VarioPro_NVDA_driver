...
#  Leonard de Ruijter, Babbage (info@babbage.com)
# Versions:
#  2019.11.11 - Conversion to Pyton 3
...
#              - created a base class VarioProModule which is
#                inherited by the classes created for each VarioPro module
...
#              - using hwIo.Serial's waitForRead method instead of time.sleep
#                (which locks the main thread of NVDA)
#              - added description to some functions
# Note: Added the possibility to output on the Status and Telephone modules.
#      But we still need to find a method to make NVDA send text specifically for the
#      Status and Telephone Braille modules.
...
# REAL INFO TYPE IDS
BAUM_VP_DEVICE_DETECTION = 0x50
BAUM_VP_DYNAMIC_DATA_BLOCK = 0x51
...
def varioProMainModuleFactory(number_cells):
	return type(f"VarioProMainModule{number_cells}", (VarioProMainModule,), {
		"number_cells": number_cells,
	})


VarioProMainModule80 = varioProMainModuleFactory(80)
VarioProMainModule64 = varioProMainModuleFactory(64)
...
class BrailleDisplayDriver(braille.BrailleDisplayDriver):
	name = "baumVarioPro"
	description = "Baum VarioPro braille displays"
	isThreadSafe = True
	...
	def braille_out_send(self, cells, moduleId):
		if cells and moduleId:
			...
			self.send_packet(BAUM_VP_DYNAMIC_DATA_BLOCK, payload)
	...
	def display(self, cells):
		...
		if self.statusModule and self.statusModule.output_handler:
			self.statusModule.output_handler(
				cells[self.mainModule.number_cells:(self.mainModule.number_cells + BAUM_STATUS_CELLS_NUMBER)],
				self.statusModule.module_id
			)
		if self.telephoneModule and self.telephoneModule.output_handler:
			main_cells = self.mainModule.number_cells + BAUM_STATUS_CELLS_NUMBER
			self.telephoneModule.output_handler(
				cells[main_cells:(main_cells + BAUM_TELEPHONE_CELLS_NUMBER)],
				self.telephoneModule.module_id
			)

	gestureMap = inputCore.GlobalGestureMap({
		"globalCommands.GlobalCommands": {
			"activateBrailleSettingsDialog": (
				"br(baumVarioPro):d1+d2+d4+d5",
				"br(baumVarioPro):smc1", "br(baumVarioPro):tmc1"
			),
			"announceDropbox": ("br(baumVarioPro):d1+d4+d5",),
			"braille_cycleCursorShape": (
				"br(baumVarioPro):d2+d3+d5+d6",
				"br(baumVarioPro):smc4",
				"br(baumVarioPro):tmc4"
			),
			...
			"braille_toFocus": (
				"br(baumVarioPro):d5+d6",
				"br(baumVarioPro):wp1"
			),
			"braille_toggleShowCursor": (
				"br(baumVarioPro):d2+d4+d5",
				"br(baumVarioPro):smc3",
				"br(baumVarioPro):tmc3"
			),
			"braille_toggleTether": (
				"br(baumVarioPro):d1+d5",
				"br(baumVarioPro):smc2",
				"br(baumVarioPro):tmc2"
			),
			...
			"kb:enter": (
				"br(baumVarioPro):d1+d2+d3+d5",
				"br(baumVarioPro):d2+d3+d5",
				"br(baumVarioPro):wp2",
				"br(baumVarioPro):wp4"
			),
			"kb:control+home": ("br(baumVarioPro):d1+d4",),
			"kb:downArrow": (
				"br(baumVarioPro):d2+d3",
				"br(baumVarioPro):wd2"
			),
			"kb:upArrow": (
				"br(baumVarioPro):d1+d2",
				"br(baumVarioPro):wu2"
			),
			...
			"moveNavigatorObjectToMouse": (
				"br(baumVarioPro):d4+d5",
				"br(baumVarioPro):tn#+tn1",
				"br(baumVarioPro):tn#+tn0"
			),
